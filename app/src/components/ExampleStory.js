...
class ExampleStory extends Component {
  ...
  getExamples() {
    ...
    return content.map((item, index) => {
      let element;
      if (item.type === 'intent') { element = this.exampleIntent(item, index); } else if (item.sequence) { element = this.exampleSimpleUtter(item, index); } else { element = this.exampleUtter(item, index); }
      return element;
    });
  }
