...
export const validationContent = (content) => {
  ...
  } if (content.length !== 0 && content[0].type !== 'intent') {
...
export const deleteContent = (state = INITIAL_STATE, action) => {
  const new_content = createArrayObjCopyOf(state.content);
...
export const { Types, Creators } = createActions({
  ...
  addIntent: (intent) => async (dispatch) => {
    try {
  ...
  getStories: (value = '') => async (dispatch) => {
    try {
