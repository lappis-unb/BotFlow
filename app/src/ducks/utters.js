...
export const addUtterContent = (state = INITIAL_STATE) => {
  const new_utter = createArrayCopyOf(state.content);
...
export const setUtterContent = (state = INITIAL_STATE, action) => {
  const content = createArrayCopyOf(state.content);
...
export const deleteUtterContent = (state = INITIAL_STATE, action) => {
  const content = createArrayCopyOf(state.content);
  const old_utter_history = createArrayCopyOf(state.content);
...
export const { Types, Creators } = createActions({
  ...
  selectUtter: (id = '', item_position = '') => async (dispatch) => {
    try {
      const response = await axios.get(UTTER_URL + id);
      await dispatch({ type: Types.SELECT_UTTER, item: response.data, item_position });
    } catch (error) {
      throw (error);
    }
  },
  getUtters: () => async (dispatch) => {
    try {
  ...
  deleteUtter: (delete_id) => async (dispatch) => {
    try {
